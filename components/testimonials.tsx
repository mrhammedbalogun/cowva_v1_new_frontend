    ...
    <section ref={ref} className="py-16 bg-[#0D1B2A] overflow-hidden">
      ...
      >
        <h2 className="text-2xl sm:text-3xl font-bold text-white">
          Trusted by <span className="text-[#00B5AD]">70+</span> Health Facilities across Nigeria
        </h2>
      </motion.div>
      ...
      <div className="relative">
        ...
        <div className="flex animate-marquee">
          {[...facilityTypes, ...facilityTypes].map((type, index) => (
            <div
              key={`${type}-${index}`}
              className="flex-shrink-0 mx-4"
            >
              <div className="px-6 py-3 bg-[#00B5AD]/5 border border-[#00B5AD]/20 rounded-full">
                <span className="text-gray-300 whitespace-nowrap">{type}</span>
              </div>
            </div>
          ))}
        </div>
